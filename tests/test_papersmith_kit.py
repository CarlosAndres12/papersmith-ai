...
def test_env_override_wins(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("PAPERSMITH_KIT_ROOT", str(checkout))
    resolved = resolve_kit_root(start_file=tmp_path / "elsewhere" / "pkg" / "kit.py")
    assert resolved == checkout


def test_dev_checkout_detection(tmp_path: Path) -> None:
    checkout = _make_checkout(tmp_path)
...
def test_bundled_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PAPERSMITH_KIT_ROOT", raising=False)
    fake_module = tmp_path / "site" / "papersmith" / "kit.py"
    resolved = resolve_kit_root(start_file=fake_module)
    assert resolved == tmp_path / "site" / "papersmith" / "_kit"


def test_invalid_kit_root_refused(tmp_path: Path) -> None:
    empty = tmp_path / "empty"
    empty.mkdir()
    with pytest.raises(SourceError, match="no skills/"):
        validate_kit_root(empty)


def test_resolve_and_validate_raises_on_bad_env(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    bad = tmp_path / "bad"
    bad.mkdir()
    monkeypatch.setenv("PAPERSMITH_KIT_ROOT", str(bad))
    with pytest.raises(SourceError):
        resolve_and_validate(start_file=tmp_path / "x" / "kit.py")


def test_walk_kit_files_filters(tmp_path: Path) -> None:
    checkout = _make_checkout(tmp_path)
...
def test_is_preserved(relpath: str, preserved: bool) -> None:
    assert manifest.is_preserved(relpath) is preserved


def test_sha256_stable_and_distinct(tmp_path: Path) -> None:
...
def test_kit_manifest_roundtrip(tmp_path: Path) -> None:
