...
def test_repo_papersmith_yaml_parses() -> None:
...
def test_quoted_scalars_keep_specials() -> None:
...
def test_unquoted_colon_in_value_is_kept() -> None:
...
def test_nested_sequences_of_mappings() -> None:
    text = """\
profiles:
  - name: x
    value: 1
...
def test_sequence_item_with_nested_block() -> None:
    data = loads("things:\n  -\n    a: 1\n    b: 2\n  -\n    a: 3\n")
    assert data["things"] == [{"a": 1, "b": 2}, {"a": 3}]


def test_document_marker_and_empty_documents() -> None:
    assert loads("---\nkey: value\n") == {"key": "value"}
...
def test_duplicate_key_refused() -> None:
    with pytest.raises(YamlliteError, match="duplicate key"):
        loads("a: 1\na: 2\n")


def test_tab_indentation_refused() -> None:
    with pytest.raises(YamlliteError, match="tabs are not allowed"):
        loads("a:\n\tb: 1\n")


def test_anchor_refused() -> None:
    with pytest.raises(YamlliteError, match="unsupported YAML construct"):
        loads("a: &anchor value\n")


def test_flow_mapping_refused() -> None:
    with pytest.raises(YamlliteError, match="flow mappings are not supported"):
        loads("a: {b: 1}\n")


def test_nested_flow_refused() -> None:
    with pytest.raises(YamlliteError, match="nested flow"):
        loads("a: [1, [2]]\n")


def test_unterminated_quote_refused() -> None:
    with pytest.raises(YamlliteError, match="unterminated quoted scalar"):
        loads('a: "never closed\n')


def test_bare_dash_refused() -> None:
    with pytest.raises(YamlliteError, match="sequence item '-' needs nested content"):
        loads("a:\n  -\nnext: 1\n")


def test_missing_colon_refused() -> None:
    with pytest.raises(YamlliteError, match="expected 'key: value'"):
        loads("just a scalar line\n")


def test_trailing_junk_after_quoted_refused() -> None:
    with pytest.raises(YamlliteError, match="trailing characters"):
        loads('a: "x" junk\n')


def test_error_reports_line_number() -> None:
    with pytest.raises(YamlliteError) as excinfo:
        loads("a: 1\nb: 2\nc: 3\nd: 4\ne: 5\nf: [1, {x}]\n")
