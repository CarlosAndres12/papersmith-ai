...
- [x] 3.4 Create `tests/seal/cases.json`: 29 cases per design D3 table, all 20 `COMMANDS`; `<TARGET>`/`<PROPOSALS>` placeholders; `--session` literals restricted to `{"seal-s1","seal-s2"}`. Verified empirically against the real CLI, case by case, before finalizing (see apply-progress): gate's E0/E1 pair needed genuinely different argv (`--authorization` present/absent), not a shared pair like the other four F3 commands.
- [x] 3.5 Build constructed-env allow-list: `PATH`, `HOME`, `PYTHONHASHSEED=0`, `PYTHONDONTWRITEBYTECODE=1`, `LC_ALL=C.UTF-8`, `TZ=UTC`, `NO_COLOR=1`, `COLUMNS=80`, `GIT_CONFIG_GLOBAL`/`SYSTEM=/dev/null`, `IMPLEMENTATION_PROPOSALS` only when case says.
- [x] 3.6 Create `tests/seal_capture.py`: argv = `shlex.split(impl.CLI_INVOCATION) + case_argv`, `shell=False`, `timeout=120`, per-case `copytree` scratch; run each case twice, refuse-and-write-nothing + unified diff on disagreement. Third placeholder `<PLAN>` beside `<TARGET>`/`<PROPOSALS>` (apply/materialize cases only): harness writes a small `plan.json` sibling to (never inside) the case's copied git target, with `"target"` injected as that case's own resolved scratch path and the other four `build_plan()` fields at their fixed (empty) values — never a statically committed plan.json, since the exact-string `target` match in `_materialize_plan_gate`/`cmd_apply` would only ever match one specific tmpdir. **Measured, not assumed**: `resolve_target` refuses `OUTSIDE_WORKSPACE` for anything not under `FORGE_ROOT / "implementations"` — the per-case scratch ROOT (not the committed corpus fixtures themselves, never used as `--target` directly) must live there, not a bare OS tempdir; `implementations/*` is gitignored, so this never touches tracked state, and every scratch dir is `shutil.rmtree`d in a `finally` block.

## Phase 4: Threat-Matrix RED Tests (before capture runs live)

- [x] 4.1 Roster validator refuses a case whose argv contains a shell metacharacter. `RosterValidationTests.test_refuses_a_case_whose_argv_contains_a_shell_metacharacter` — plus a dedicated guard test that a legitimate `$$`-bearing case (compose's real LaTeX) is NOT refused.
- [x] 4.2 Env-builder test: emits no key outside the allow-list. `EnvAllowListTests` (2 tests).
- [x] 4.3 Writing-case guard: refuses a case whose `--target` is not under the scratch root. `RosterValidationTests.test_refuses_a_case_whose_target_is_not_the_placeholder` — enforced by requiring the literal `<TARGET>` placeholder, which the harness alone resolves into a fresh scratch copy.
- [x] 4.4 Confirm every `subprocess.run` call carries `timeout=120`; a timeout is a hard failure, never routed to `unsealed.json`. `SubprocessTimeoutTests`.

## Phase 5: Normalizer Mutation Proofs (RED before GREEN, D4 table)

- [x] 5.1 N1 reach: disable N1 → two timestamp-only-differing samples stay distinct (red); re-enable → collapse (green). Guard: `2026-09-10` vs `2026-09-11` stay distinct. `NormalizerMutationTests.test_n1_reach_two_timestamp_only_samples_collapse` / `test_n1_guard_bare_dates_stay_distinct`.
- [x] 5.2 N2 reach/guard: tmpdir-only difference collapses; `src/Seal/__init__.py` vs `steps.py` and `/usr/bin/x` stay distinct. `test_n2_reach_tmpdir_only_difference_collapses` / `test_n2_guard_sibling_modules_stay_distinct` / `test_n2_guard_unrelated_absolute_path_untouched`.
- [x] 5.3 N3 reach/guard: interpreter-token-only difference collapses (two samples differing only in `sys.executable`'s path collapse to `<PYTHON>`); `python3 other.py` vs `python3 another.py` stay distinct — proving a `CLI_PATH` difference survives both N3 and N2. PLUS `test_the_sealed_cli_path_names_the_launcher`: pinned assertion (N4's idiom) that every `CLI_INVOCATION`-derived case's normalized text contains the literal `<FORGE>/.claude/skills/proposal-implementation/scripts/implementation_cli.py`; mutation — substitute a different resolved path into the raw pre-normalization text → red. This is the change's single most load-bearing mutation (design.md D4). **Verified live**: `gate-e1`/`step`/`offer-e1`'s real captured stdout all carry the literal launcher path post-normalization, pasted in apply-progress.
- [x] 5.4 N4 pinned-not-erased: change one case's `--session` literal → its digest moves (proves reach); assert no value outside `{"seal-s1","seal-s2"}` appears. Used `defect` (echoes `session` directly, only other varying field `at` is N1-normalized uniformly, isolating the reach cleanly).
- [x] 5.5 N5 reach/guard: 40-hex collapses to `<GITSHA>`; a 64-hex `sha256` survives; no 7–12 hex short sha in any golden (scanned every captured case's live normalized text).
- [x] 5.6 N6 (deliberately unnormalized): flip one byte in `src/Seal/__init__.py` inside the corpus → case-12 digest moves; a future `<DIGEST>` normalizer would make this red. **Adapted, measured not assumed**: production code SKIPS `__init__.py` for provenance (`cmd_verify`'s own module loop), so nothing there is reachable this way; `suiteDigest`/`source_digest` are never reached by this corpus either (`step` pinned at `INTERPRETER_ABSENT`); `admit`'s own `revisionSha256` lands in `tests/admissibility.json` inside the target, never stdout. Used the field and command this corpus actually surfaces a content digest through: flip one byte of the REVISION TEXT → `position-e1`'s digest (which directly echoes `revisionSha256`) moves.
- [x] 5.7 `test_normalizer_order_is_pinned`: `NORMALIZERS` identity tuple, N3 before N2.

## Phase 6: Capture The Seal (post-F3 code)

- [x] 6.1 Run `tests/seal_capture.py`; paste full output; on success it writes `tests/seal/digests.json` and `tests/seal/unsealed.json` — commit both.

  ```
  Capturing 29 cases, twice, in two independently-built corpora...
  Captured 28 sealed case(s). Wrote .../tests/seal/digests.json and
  .../tests/seal/unsealed.json (1 unsealed: ['propose']).
  ```

  **Finding, closed inline via D6's own mechanism**: `propose`'s own campaign
  digest (`cmd_propose`'s `digest` field) is `sha256` over a JSON payload that
  embeds `_now_iso8601()` at second precision, so it differs across two
  immediate runs even with source/session/job/rationale fixed — none of the
  six normalized sources reach an opaque hash DERIVED FROM, but not textually
  equal to, a timestamp. Every other byte already agreed between the two
  captures; only `digest` moved. Declared unsealed with a reason
  (`tests/seal_capture.py`'s `KNOWN_UNSEALED_REASONS`, mirrored into
  `unsealed.json`) rather than a seventh normalizer, because spec.md's own
  Normalization requirement names exactly six sources and `unsealed.json` is
...
- [x] 7.1 `tests/test_implementation_seal.py`: run each of 29 cases once, normalize, compare to `digests.json`. `SealComparisonTests.test_every_sealed_case_matches_its_golden` (28 sealed cases, `propose` excluded via the membership tests below).
- [x] 7.2 Coverage tests (over captured *output*, not fixture): `test_provenance_reaches_the_output`, `test_findings_reach_the_output` (all 3 of inline/deferred/settled + `introduces`), `test_corpus_provably_exercises_data_present_and_absent` (construction-level: fixture A's `Seal/Data/` exists, fixture B's does not — replaces the falsified `test_data_absence_is_visible`, per coordinator decision and design.md D3's correction), `test_both_revision_families_and_a_tie_are_exercised`, `test_every_f3_site_is_sealed_in_both_env_states` (exact set `{admit,position,gate,offer,close}×{E0,E1}`). All pass.
- [x] 7.3 Membership tests: `test_every_case_is_either_sealed_or_declared_unsealed`, `test_the_unsealed_set_is_exactly_its_declared_membership` (`EXPECTED_UNSEALED = frozenset({"propose"})` — see 6.1's finding), `test_the_case_roster_covers_the_command_roster_exactly`, `test_every_unsealed_entry_states_a_reason`.
- [x] 7.4 `test_corpus_fingerprint_matches`: `sha256(corpus.py)` vs stored; trim a case → red. Both pass (`CorpusFingerprintTests`).
- [x] 7.5 **Seal mutation proof**: flip one byte of one stored golden stdout → comparison goes red; demonstrate, paste, then revert the mutation. `SealMutationProofTests.test_flipping_one_byte_of_a_golden_turns_the_comparison_red` — demonstrated in memory (a copied dict, never the on-disk `digests.json`), so revert is automatic and provable within the same test.
- [x] 7.6 Measure discover-delta: `time` `unittest discover -s tests` before/after this phase; paste both; ceiling ≤60s. Baseline (session start, 2783 tests): `587.935s`. After Phase 3-7 (2821 tests, +38 new): `458.560s`. Both `OK (skipped=6)`, zero failures. The full-suite wall-clock is dominated by system load noise between runs (528.710s was also measured mid-session, after only 2 new tests) — the new file's OWN incremental cost, isolated (`.venv/bin/python -m unittest tests.test_implementation_seal`), was measured directly at `Ran 38 tests in 4.941s` — well within the 60s ceiling regardless of noise.
- [x] 7.7 Open question — `materialize` (case 28): run once; if slow or writes outside scratch, pin at a refusal leg like `step`/`env` and update `cases.json`; else keep its current non-destructive leg. **Resolved**: `materialize --stage scaffold` (with `--plan`/`--seed`) runs instantly and writes ONLY inside the scratch target (real kit files copied from `SKILL_ROOT/assets/kit/`, never outside). Kept at its current non-destructive leg — no pin needed. Recorded in design.md's Open Questions.
- [x] 7.8 Confirm python suite: `Ran 2783+K ... OK (skipped=6)`, zero new skips, zero failures; paste. `Ran 2821 tests in 458.560s` / `OK (skipped=6)` — K=38, 0 failures, 0 new skips.
