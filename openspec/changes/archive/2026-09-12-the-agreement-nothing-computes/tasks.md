...
- **Task count: 99**, all checked. The header has now been wrong twice: the authored 78, and the 100 this line itself claimed after the R1/R2 reconciliation. The 100 was counted before apply merged 1.12/1.13 into one checkbox; the same command, re-run at the end of phase 6, gives 99. Derivation: `rg -o '^\s*- \[[ x]\] [0-9]+\.[0-9]+[a-z]?' | wc -l`. Completeness and count-accuracy are separate properties: every box was genuinely checked while this total was stale.
