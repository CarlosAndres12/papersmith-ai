...
- [x] 6.5 Re-derive the refusal roster: execute `reachable_paper_refusal_codes()` and record the MEASURED number in the apply/verify report; never write a predicted number into any artifact ahead of this measurement. Measured live: **152** (unchanged from Phase 5 — this phase adds no code).
- [x] 6.6 Confirm `resolve_section_index`'s `outline` return (byte offsets, task 2.6) is importable and callable by a sibling module with no duplication of the marker → lineage → `segment_markdown` chain — a smoke import/call proving the shape, not a behavior change. Confirmed: `paper_graph.resolve_section_index(source_roots, root, lineage) -> tuple`, imported and called by `paper_cli.compute_separation`.
- [x] 6.7 Confirm zero edits landed in `paper_leak.py` or `paper_write.py` — `git diff main -- .claude/skills/paper-writing/scripts/paper_leak.py .claude/skills/paper-writing/scripts/paper_write.py` is empty. Confirmed empty (0 lines).
- [x] 6.8 Confirm nothing was deleted — `git log --diff-filter=D` shows no removed files for this branch. Confirmed empty (`git log --diff-filter=D --name-only main..` returns nothing).
- [x] 6.9 Run the Python suite in FOUR chunks (full discover exceeds the 600s foreground timeout, and background runs are killed by memory pressure on this machine): split `.venv/bin/python -m unittest discover -s tests -p 'test_*.py'` by module into four roughly-equal chunks and run each in the foreground. Ran sequentially, never concurrently; chunk 3 used `PYTHONPATH=tests` for `test_orphan_sweep.py`. Totals: 1653 + 700 + 996 + 1021 = 4370 tests, 1 known failure, 3 skipped, rest passed.
- [x] 6.10 Run `npm test`; confirm 640/640. Confirmed: 640 pass, 0 fail.
- [x] 6.11 Confirm the Python result carries exactly ONE known pre-existing failure (`test_proposal_implementation.ForgeVocabularyDerivedGuardTests.test_rule_b_finds_no_target_vocabulary_in_the_forge`, over the generic word "mechanisms"); any second failure is this change's and blocks. Confirmed: exactly one failure, in chunk 1, that exact test.
- [x] 6.12 Update `design.md`'s roster line with the measured post-change count (task 6.5's result) and confirm no artifact still carries a forecast number. Updated: 144 (pre-change) → 152 (measured post-change, task 6.5's own number).
