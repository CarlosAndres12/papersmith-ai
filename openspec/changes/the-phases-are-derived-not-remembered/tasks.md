...
| 4 | `.venv/bin/python -m unittest tests.test_paper_contract -v -k Chain` (NOT `test_paper_writing`: `InternalChainTests` lives in `test_paper_contract.py`, and the wrong module reports `NO TESTS RAN` with exit 0 — a false pass) | `paper_cli.py contract` (zero refusals on the real corpus) | Revert `_verify_internal_chain` + the ten header `after` additions; `_verify_after_transcription` still guards edges added |
