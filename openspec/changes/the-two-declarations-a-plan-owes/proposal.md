...
| 7 | The seeds check demands the literal word `seeds` plus a digit | **Accept synonyms** — `seeds`, `semillas`, `initialisations`, `initializations`, `runs`, each with its digit. The design's own conclusion is that these lists must bias toward a false pass over a false block; the literal form refuses "five random initialisations", which is a correct line |
