...
Presence of both closings (intro block 3, related-work block 4) and
front-count/front-list equality MUST be computed mechanically. "The same
thing at different depths" MUST NOT be computed; the payload MUST publish
both closing texts and both front lists verbatim, and the reading's own
verdict MUST be `unmeasured` until a human rules. `verify` MUST NOT answer
or gate on that reading (resolves proposal question 4: nobody closes it here).
