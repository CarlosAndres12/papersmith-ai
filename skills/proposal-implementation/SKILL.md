...
### Flow A branch — the work already exists as an external folder

When the user points at a folder instead of a URL — prior code, a paper
draft, or both, sitting outside the workspace — do not clone and do not copy
it by hand. Run `adopt` survey over it, which copies the tree under
`implementations/<repo>`, git-inits it, and writes `adoption-plan.json`
beside the copy:

`adopt --source <folder> --target implementations/<repo> --name <Name>`

The survey normalizes the name first, detects the paper draft (zero means
code-only, one is proposed as the v1 base, several refuse rather than guess),
and runs the plan machinery over the copy — reorganization with its decision
count and scale, the thirteen scaffold gaps, unclassified files, conflicts,
and the kit destinations the copy already holds. Present all of it, exactly
as step 4 presents a plan: the reorganization list, the paper candidate, the
scaffold gaps, the adopted files. A `scale: "large"` list is presented, never
applied blind, and no flag overrides that gate.

**[GATE]** On approval, `adopt --apply --plan <path to the adoption plan>`
revalidates the surveyed bytes, places the draft as the managed v1, lands
the migration in one commit, writes the scaffold stage and records each
adopted file with the degraded guarantee — the record names who wrote the
bytes, not that they came from the kit. Continue at step 7: the name is
settled, the scaffold is written, the paper stands as v1, and what remains
is the authorization to implement, the object map, and the code.
...
One hundred and fourteen distinct codes are reachable from the ten gating
commands — `apply`, `admit`, `gate`, `offer`, `close`, `step`, `settle`,
`materialize`, `position`, `adopt` (`GATING_COMMANDS`). **Reachable from, not raised
...
- **No — a *work state*** (64 codes). Somebody must act on the repository, so
